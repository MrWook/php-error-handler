<?php
	 ...
	 *
	 * @param int $error_memory set the internal error memory save for php overflows if the $log_object or the $log_info is to big it need to be increased (Default is 524288 bytes)
	 *
	 * @param bool $enable_reporting set if the custom handling should be activated need to be true to log the error's (Default is true)
	 *
	...
	class ErrorHandler{
		...
		public function log_error($error_lvl, $error_msg, $error_file = null, $error_line = null, $error_context = null){
			...
			//set error context
			else if(isset($error_context) && strlen($error_context) > 0)
